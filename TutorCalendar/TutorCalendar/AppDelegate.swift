...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func skipLoginPage()  -> Bool{
        ...
        util.debug(2, args: "skiploginpage")
        do{
            try
                
                loggedInUserProfile = managedObjectCTX.executeFetchRequest(fetchRequest) as! [UserProfileObject]
            util.debug(2, args: loggedInUserProfile)
            if loggedInUserProfile.count > 0 {
                util.studentID = Int(loggedInUserProfile[0].profileID)
            }
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        util.debug(2, args: "shouldskiploginpage")
        if skipLoginPage(){
            let storyboard  = UIStoryboard(name: "Main", bundle: nil)
            //            let root = storyboard.instantiateViewControllerWithIdentifier("root")
            //                as! UINavigationController
            let destination = storyboard.instantiateViewControllerWithIdentifier("NewRoot") //as? UINavigationController
            //            root.pushViewController(destination, animated: false)
            //            self.window?.rootViewController = root
            util.debug(2, args: destination)
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "DeAnza.TutorCalendar" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("TutorCalendar", withExtension: "momd")!
